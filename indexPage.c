...
int indexPage(const char *url)
{
  ...
  {
    if ((contents_buf[i] == ' ') || (contents_buf[i] == '\n'))
    {
      if (isValid)
      {
        contents_buf[i] = '\0';
      ...
      current = contents_buf + i + 1;
    }
    else if (isalpha(contents_buf[i]))
    {
      isValid = 1;
    }
    else
  ...
  if (isValid)
  {
    addWordOccurrence(root, current, contents_len);
    return 0;
  }
  else
  {
    printf("Error on Index Page");
    return 1;
  }
}
